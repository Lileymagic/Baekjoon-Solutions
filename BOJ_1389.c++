...
// dfs 탐색
int DFS(const vector<vector<int>>& graph, vector<int>& visited, int current, int target, int stack) {
    visited[current] = 1;
    int result = INT_MAX;
    int tmp;
    ...
    for(int i = 0; i < graph[current].size(); i++) {
        ...
        if (visited[graph[current][i]] == 0) {
            ...
            tmp = DFS(graph, visited, graph[current][i], target, stack + 1);
